...
impl DiffLayer {
    ...
    ///
    /// Takes the prefix by reference (`&[u8]`) so per-layer scans in
    /// `DiffLayers::get_trie_nodes` don't have to clone the lookup key
    /// 256 times. Saves ~2-3% CPU on the state-root path under load.
...
impl DiffLayers {
    ...
    /// Get a trie node by prefix.
    ///
    /// Walks layers newest-first; the prefix is passed by reference so no
    /// `Vec` clone is needed at each layer (caller can pass either an owned
    /// `Vec<u8>` via deref or a borrowed slice directly).
    ///
    ...
    /// builder can attribute this query to the DiffLayer tier.
    pub fn get_trie_nodes(&self, prefix: &[u8]) -> Option<Arc<TrieNode>> {
